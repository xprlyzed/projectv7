...
const tabs = computed(() => [
    ...
    { key: 'paid',     label: `Ödenen (${props.stats.paid})` },
    { key: 'rejected', label: `Reddedilen (${props.stats.rejected})` },
]);

const cur = computed(() => props.filters.status || 'pending');
function tabHref(key) {
    return route('admin.withdrawals.index', { status: key });
}

const busy = ref(null);

function approve(row) {
    const doApprove = (reference) => {
        if (busy.value) return;
        busy.value = row.id;
        router.post(row.approve_url, { payment_reference: reference || '' }, { preserveScroll: true, onFinish: () => (busy.value = null) });
    };
    if (typeof window.Swal !== 'undefined') {
        window.Swal.fire({
            title: 'Ödeme yapıldı mı?',
            html: `<strong>${row.user_name}</strong> · <strong>${row.amount}</strong><br><small>${row.iban}</small>`,
            input: 'text',
            inputPlaceholder: 'Ödeme referansı (opsiyonel)',
            icon: 'question', showCancelButton: true,
            confirmButtonText: 'Onayla & Ödendi İşaretle', cancelButtonText: 'Vazgeç',
            reverseButtons: true, confirmButtonColor: '#10b981', heightAuto: false,
        }).then((r) => { if (r.isConfirmed) doApprove((r.value || '').trim()); });
    } else if (confirm('Talep onaylanıp ödendi olarak işaretlensin mi?')) {
        doApprove('');
    }
}
...
function reject(row) {
    ...
    if (typeof window.Swal !== 'undefined') {
        window.Swal.fire({
            title: 'Talebi reddet',
            text: 'Rezerve edilen tutar kullanıcıya geri yüklenecek.',
            input: 'textarea',
...
<template>
    <Head title="Para Çekme Talepleri" />
    <div class="dash-wrap py-4" data-testid="admin-withdrawals-page">

        <div class="admin-toolbar dash-hero">
            <div>
                <div class="toolbar-title">Para Çekme Talepleri</div>
                <div class="dash-hero-sub">IBAN'a ödeme yaptıktan sonra "Onayla & Ödendi" kullanın. Ret durumunda bakiye kullanıcıya geri yüklenir.</div>
            </div>
        </div>

        <div v-if="flash.success" class="alert alert-success" data-testid="withdrawals-flash-success">{{ flash.success }}</div>
        <div v-if="flash.error" class="alert alert-danger" data-testid="withdrawals-flash-error">{{ flash.error }}</div>

        <div class="admin-card orders-tabs" data-testid="admin-withdrawals-tabs">
            <Link v-for="t in tabs" :key="t.key" :href="tabHref(t.key)"
                  :class="cur === t.key ? 'btn-admin-pri' : 'btn-admin-ghost'"
                  :data-testid="`admin-withdrawals-tab-${t.key}`">{{ t.label }}</Link>
        </div>

        <div class="admin-card" data-testid="admin-withdrawals-card">
            <template v-if="!requests.data.length">
                <div class="pf-empty" data-testid="withdrawals-empty">
                    <div class="pf-empty-icon"><i class="bi bi-inbox"></i></div>
                    <div class="pf-empty-title">Kayıt yok</div>
                    <div class="pf-empty-sub">Bu durumda bekleyen para çekme talebi bulunmuyor.</div>
                </div>
            </template>
            <template v-else>
                                ...
                                <td>
                                    <div v-if="row.status === 'pending'" class="d-flex gap-1 justify-content-end">
                                        <button type="button" class="btn-admin-success" :disabled="busy === row.id"
                                                @click="approve(row)" :data-testid="`withdrawal-approve-${row.id}`">
                                            <i class="bi bi-check-lg"></i> Onayla &amp; Ödendi
                                        </button>
                                        <button type="button" class="btn-admin-danger" :disabled="busy === row.id"
                                                @click="reject(row)" :data-testid="`withdrawal-reject-${row.id}`">
